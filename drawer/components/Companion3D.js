...
function buildProceduralMesh() {
  // A smooth sphere rather than the faceted icosahedron — smoothShading
  // (flatShading: false) plus a higher segment count is what actually
  // reads as a round orb instead of a geodesic/faceted ball. Morph
  // displacement amplitude is also toned down from the icosahedron
  // version (0.55/0.85 looked fine as jagged facets, but the same
  // displacement on smooth-shaded geometry read as lumpy/blobby rather
  // than a gentle breathing distortion).
  ...
  for (let i = 0; i < posAttr.count; i++) {
    ...
    const dA = (rand() - 0.5) * 0.22;
    const dB = (rand() - 0.5) * 0.32;
