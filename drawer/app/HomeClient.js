...
function LinkedInIcon({ size = 22 }) {
  return (
    <span style={{ fontFamily: 'var(--font-space-grotesk)', fontWeight: 700, fontSize: size, lineHeight: 1, color: 'currentColor' }} aria-hidden="true">
      in
    </span>
  );
}
...
function SocialCard({ platform, label, Icon, iconSize = 30 }) {
  return (
  ...
  return (
    <section ref={ref} className={`closing-social ${revealed ? 'is-revealed' : ''}`}>
      <div className="social-stack">
        <SocialCard platform="instagram" label="Instagram" Icon={SocialIcon} iconSize={30} />
        <SocialCard platform="linkedin" label="LinkedIn" Icon={LinkedInIcon} iconSize={26} />
      </div>
    </section>
  );
}
        ...
        <>
          ...
          <div className="slide-deck">
            {SLIDES.map((slide, i) => (
              <div
                key={i}
                className={`slide ${i === activeSlide ? 'is-active' : i > activeSlide ? 'is-below' : 'is-above'} ${slide.type === 'intro' ? 'intro-slide' : ''}`}
